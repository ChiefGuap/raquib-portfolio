...
const MacOSDock: React.FC<MacOSDockProps> = ({ 
  ...
  const [bounceStates, setBounceStates] = useState<boolean[]>(apps.map(() => false));
  ...
  const handleAppClick = (appId: string, index: number) => {
    // Trigger bounce state
    setBounceStates(prev => {
    ...
    >
      ...
      >
        {apps.map((app, index) => {
          ...
          const isBouncing = bounceStates[index];
          const bounceOffset = isBouncing ? Math.max(-12, -baseIconSize * 0.2) : 0;
          ...
          return (
            <div
              ...
              style={{
                ...
                top: '50%',
                transform: `translateY(calc(-50% + ${bounceOffset}px))`,
                width: `${scaledSize}px`,
